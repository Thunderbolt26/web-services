...
public class Browser {
    ...
    public static void main(String args[]) {
        Browser sp = new Browser();
        sp.Browse(args);
    }

    public void browse(BrowserQuery query) {
        String token = GetAuthKey("uddi", "uddi");
        wsdl=null;
        switch (query.getSubject()) {
            case BUSINESS:
                browseBusiness(query.getSearchString(), token);
                break;
            case SERVICE:
                browseService(query.getSearchString(), token);
                break;
            case WSDL:
                browseWSDLbyServiceKey(query.getSearchString(), token);
                break;
            default:
    ...
    private void browseBusiness(String searchString, String token) {
        try {
            BusinessList findBusiness = GetBusinessList(token, searchString);
    ...
    private void browseService(String searchString, String token) {
        try {
            ServiceList findService = GetServiceList(token, searchString);
            PrintServiceInfo(findService.getServiceInfos());
            security.discardAuthToken(new DiscardAuthToken(token));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void browseWSDLbyServiceKey(String searchString, String token) {
        try {
            getWSDLByKey(token, searchString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void Browse(String[] args) {
        try {
            ...
            String token = GetAuthKey("uddi", "uddi");
            BusinessList findBusiness = GetBusinessList(token, UDDIConstants.WILDCARD);
            PrintBusinessInfo(findBusiness.getBusinessInfos());
            PrintBusinessDetails(findBusiness.getBusinessInfos(), token);
            PrintServiceDetailsByBusiness(findBusiness.getBusinessInfos(), token);
     ...
     * @throws Exception
     */
    private BusinessList GetBusinessList(String token, String searchString) throws Exception {
        ...
        searchname.setValue(searchString);
    ...
    private ServiceList GetServiceList(String token, String searchString) throws Exception {
        ...
        searchname.setValue(searchString);
        fb.getName().add(searchname);
        ServiceList findServices = inquiry.findService(fb);
        return findServices;
    }

    private void getWSDLByKey(String token, String key) throws Exception {
        ...
        try {
            serviceDetail = inquiry.getServiceDetail(fs);
        } catch (Exception e) {
        ...
        try {
            bindingTemplates = serviceDetail.getBusinessService().get(0).getBindingTemplates();
        } catch (NullPointerException e) {
            return;
        }
        if (bindingTemplates == null) return;
            ...
            if (bindingTemplates.getBindingTemplate().get(i).getAccessPoint() != null) {
                ...
                if (bindingTemplates.getBindingTemplate().get(i).getAccessPoint().getUseType() != null) {
                    if (bindingTemplates.getBindingTemplate().get(i).getAccessPoint().getUseType().equalsIgnoreCase(AccessPointType.WSDL_DEPLOYMENT.toString())) {
                        wsdl = bindingTemplates.getBindingTemplate().get(i).getAccessPoint().getValue();
                        return;
    ...
    private String getWSDL(ServiceInfo serviceInfo) {
        if(serviceInfo!=null) {
